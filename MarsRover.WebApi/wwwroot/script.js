...
function postDateInputFile() {
    ...
    request.open('POST', 'api/v1/dates');
    request.onload = () => {
...
function addedListUl(addedList) {
    ...
    for (var i = 0; i < addedList.length; i++) {
        ...
        addedListLi.innerHTML = new Date(addedList[i]).toLocaleDateString();
...
function getRoverManifests() {
    ...
    request.open('GET', 'api/v1/rovers');
    request.onload = () => {
...
function populateRoverList(rovers) {
    ...
    if (rovers.length > 0) {
        ...
        var option = document.createElement('option');
        option.innerHTML = 'Unable to retrieve rovers';
        option.value = '';
...
function populatePhotoDates(photoDates) {
    ...
    if (photoDates.length > 0) {
        ...
        var option = document.createElement('option');
        option.innerHTML = 'Please upload a date file';
        option.value = '';
        photoDatesDropDown.appendChild(option);
    }
...
function getRoverPhotos(roverId, photoDate) {
    ...
    request.open('GET', 'api/v1/rovers/' + roverId + '/photos?photoDate=' + photoDate);
    request.onload = () => {
